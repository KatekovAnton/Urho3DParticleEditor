...
struct Point
{
  ...
  /** Parse strings generated by Point::str() */
...
public: // housekeeping
  /** Make a deep copy of the Grid. New & old Hexes are not interchangeable. */
...
public:
  ...
  void *_userData = nullptr;
  Point _centre; // for caching
  
  template <typename T1, typename T2>
  static inline Point centre(T1 i, T2 j){
      return { I / (2 - j % 2) + I * i, K + J * j };
  }
...
/** Calculates a first step to got to hex.*/
Direction
directionStep(const Hex* from, const Hex* to);
...
/** TRUE iff s is connected. (If true, then s can be converted into an Area.) */
bool
