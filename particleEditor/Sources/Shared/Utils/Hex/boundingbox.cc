...
BoundingBox::BoundingBox(const std::set<Hex*>& a, bool frame_) //throw (hex::invalid_argument)
{
  if(a.empty())
...
{
  const hex::Point& p0 = hex0_->centre();
  hex::Point p1 = hex1_->centre();
  if(hex1_->j != hex0_->j)
  {
    p1.x += (hex::I / (hex1_->j % 2 + 1.0f));
  }
