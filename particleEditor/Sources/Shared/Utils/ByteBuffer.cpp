...
ByteStreamBuffer::ByteStreamBuffer(ByteBuffer *source)
:_source(source)
,_offset(0)
{
    this->setg(reinterpret_cast<char *>(_source->getPointer()), reinterpret_cast<char *>(_source->getPointer()), reinterpret_cast<char *>(_source->getPointer() + _source->getDataSize()));
}

ByteStreamBuffer::ByteStreamBuffer(ByteBuffer *source, size_t offset)
:_source(source)
,_offset(offset)
{
    this->setg(reinterpret_cast<char *>(_source->getPointer() + offset), reinterpret_cast<char *>(_source->getPointer() + offset), reinterpret_cast<char *>(_source->getPointer()  + _source->getDataSize()));
}
